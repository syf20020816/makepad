...
use keyframe::{functions::EaseIn, AnimationSequence};
use rust_pixel::{
    algorithm::draw_bezier_curves,
    context::Context,
    event::{event_emit, Event, KeyCode},
    game::Model,
    util::{ParticleSystem, ParticleSystemInfo, PointF32, Rand},
};

pub const MYAPPW: u16 = 80;
pub const MYAPPH: u16 = 40;
...
pub struct SimpleModel {
    // SimpleData defined in myapp/lib/src/lib.rs
    pub data: SimpleData,
...
impl Model for SimpleModel {
    fn init(&mut self, _context: &mut Context) {
        let in_points = [
            ...
            PointF32 {
                x: MYAPPW as f32 * 16.0,
                y: MYAPPH as f32 * 16.0,
            },
        ];
        let num = 100;
        ...
        for e in &es {
            match e {
                Event::Key(key) => match key.code {
                    ...
                    _ => {
                        context.state = MyappState::Normal as u8;
                    }
                },
                _ => {}
            }
