...
impl ImageBuffer {
    ...
    pub fn from_png(data: &[u8]) -> Result<Self, ImageError> {
        ...
        if decoder.is_animated() {
            ...
            let mut output = vec![0; info.width * info.height * components];
            ...
            let total_height =
                ((actl_info.num_frames as usize / fits_horizontal) + 1) * info.height;
            ...
            while decoder.more_frames() {
                ...
                // call post process
                post_process_image(&info, colorspace, &frame, &pix, None, &mut output, None)
                    .unwrap();
                match components {
                    3 => {
                        // copy output in
            ...
            match decoder.decode() {
                ...
                Err(err) => Err(ImageError::PngDecode(err)),
            }
        }
    }
                ...
                match f.read_to_end(&mut data) {
                    Ok(_len) => {
                        if image_path.ends_with(".jpg") {
                            ...
                            error!(
                                "load_image_file_by_path: Image format not supported {}",
                                image_path
                            );
            ...
            match cx.take_dependency(image_path) {
                Ok(data) => {
                    if image_path.ends_with(".jpg") {
                        ...
                        error!(
                            "load_image_dep_by_path: Image format not supported {}",
                            image_path
                        );
                        Err(ImageError::UnsupportedFormat)
                    }
